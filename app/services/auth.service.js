...
class AuthService {
  ...
  async signIn(email, password, user_role) {
    let usr;
    ...
    try {
      await User.findOne({
        ...
        user.all_roles.includes(user_role)
          ...
          : Profile.update(
              { p_role: user.default_role },
              { where: { userId: user.id } }
            );
        return (usr = user);
      });
      if (!usr || usr.length === 0) {
        throw (400, { message: "User not found [Auth service]" });
      }
      ...
      throw {
        status: 400,
        message: "Can't find user, was been input invalid data",
      };
    }
  }
